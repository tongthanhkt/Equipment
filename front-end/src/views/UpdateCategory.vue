...
@Options({})
export default class UpdateCategory extends Vue {
  @Prop() id!: number;
  ...
  @Emit("changeUpdateCategoryShow")
  changeShow() {}
  mounted() {
    this.retrieveCategory();
  }
  async retrieveCategory() {
    const response = await CategoryService.getCategoryDetail(this.id);
