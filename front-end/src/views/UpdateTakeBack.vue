<template>
  ...
  >
    ...
    >
      ...
      >
        Chỉnh sửa thông tin thu hồi - {{ id }}
      </h1>
      <button
        class="
          place-self-end
      ...
      <div class="pl-3 text-sm grid grid-cols-3 grid-flow-row mt-2 mb-3.5">
        ...
        <div>
          <!-- <input
                ...
                placeholder=""
                v-model="record.take_back_person"
              /> -->
          <v-select
            class="
              mx-1
            ...
            <div v-for="(file_name, index) in currentFileName" :key="index">
              <div>
                ...
                >
                  ...
                  <span class="close px-2 py-1" @click="deleteCurrentFile(index)"
                    >&times;</span
                  >
                </div>
              </div>
...
export default class UpdateTakeBack extends Vue {
  ...
  async created() {
    ...
    this.options.push(this.take_back_person);
